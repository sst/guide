...
Serverless Framework allows you to create stages for your project to deploy to. Stages are useful for creating environments for testing and development. Typically you create a staging environment that is an independent clone of your production environment. This allows you test and ensure that the version of code that you are about to deploy is good to go.
...
- Using the API Gateway built-in stages
  ...
  You can create multiple stages within a single API Gateway project. Stages within the same project share the same endpoint host, but have a different path. For example, say you have a stage called 'prod' with endpoint.
...
- Separate APIs for each stage
  ...
  Note the 'dev' stage carries a different endpoint host since it belongs to a different project. This is the approach Serverless Framework takes to configure stages for your Serverless project. We will look at this in detail below.

- Separate AWS account for each stage

  Just like how having each stage being separate APIs give us more flexibility to fine tune the IAM policy. We can take it a step further and create the API project in a different AWS account. Most companies don't want their production infrastructure in the same account as their development infrastructure, as it reduces any chances where developers accidentally edit/delete production resources. We go in to more detail on how to deploy to multiple AWS accounts using different AWS profiles in the [multiple credentials](XXXXXXXX) chapter.
...
We went over the concept of environment variables in the chapter on [environment variables](XXXXXXX). Let's extend that to specify variables based on the stage we are deploying to.
