...
우리가 여기에서 하는 대부분의 작업은 상당히 간단하므로 빨리 넘어 가겠습니다.

1. 사용자에게 확인 코드를 입력하는 양식을 보여줘야하기 때문에 사용자 정보가 있는지 여부에 따라 두 가지 양식을 조건부로 렌더링합니다.
...
4. 이메일과 확인코드 필드에 `autoFocus` 플래그를 설정합니다.
